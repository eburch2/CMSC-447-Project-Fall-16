...
if(count($userInfo) > 0) {
	...
	if($umbcid != "" && $username != "" && $captcha != "") {
		...
		$row = $db1 -> select("SELECT COUNT(*) FROM users WHERE username = \"" . bin2hex($username) . "\" AND umbcid = \"" . $umbcid . "\" AND status >= 0 LIMIT 1 ");
		if($row[0]["COUNT(*)"] > 0) {
			$newPassword = "";
			$options = array();
			for($i=48; $i < 58; $i++) {
			...
			array_push($options, chr(33));
			array_push($options, chr(64));
			array_push($options, chr(35));
			array_push($options, chr(36));
			array_push($options, chr(37));
			
			for($i=0; $i < 20; $i++) {
				$newPassword .= $options[rand(0, count($options)) - 1];
			}
			if(!preg_match("/.*[!@#\$%].*$/", $newPassword)) {
				$newPassword .= $options[rand(62, count($options)) - 1];
			}
...
if($errorMessage != "") {
	...
	if($successful == true) {
		...
		?>
		<a href="resetpass.php"><input class="button2" type="button" value="Try resetting Password"/></a>
		<?php
	}
