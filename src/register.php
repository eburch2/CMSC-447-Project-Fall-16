		...
		if(count(getStudentInfo($umbcid)) > 0) {
			...
			if($row[0]["COUNT(*)"] > 0) {
				...
				if($row[0]["COUNT(*)"] > 0) {
					...
					$randomConfirm = randomHex(75);
					$hashPass = hashPassword($password);
					// insert the record into the user database
					$query1 = "INSERT INTO users (umbcid, username, password, firstname, lastname, confirmation, lastaccess, datecreated) ";
					$query1 .= "VALUES ('" . $umbcid . "', '". bin2hex($username) ."', '" . bin2hex($hashPass) . "', '" . bin2hex($firstname) . "', '"  . bin2hex($lastname) . "', ";
					$query1 .= "'" . $randomConfirm . "', SYSDATE(), SYSDATE()) ";
					//echo $query1;
