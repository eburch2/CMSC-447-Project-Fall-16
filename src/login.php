...
if(count($userInfo) > 0) {
	...
	if(count($rows[0]) > 0) {
		...
		if($attempts > $maxAttempt) {
			$failedLogin = true;
			$errorMessage .= $_SERVER["REMOTE_ADDR"] . " exceeded login attempts.<br/>\n";
		}
	}
	...
	if($username != "" && $password != "" && $attempts <= $maxAttempt) {
		...
		//echo bin2hex($hashPass);
		// data in the database is stored as hex to prevent SQL injection
		$rows = $db1 -> select("SELECT userid, umbcid, firstname, lastname FROM users WHERE password = \"" . bin2hex($hashPass) . "\" AND username = \"" . bin2hex($username). "\" AND status >= 10 LIMIT 1");
		//echo count($rows[0]);
		if(count($rows[0]) > 0) {
			$userLoggedIn = true;
			...
			$_SESSION["umbcbazaar_firstname"] = $rows[0]["firstname"];
			$_SESSION["umbcbazaar_lastname"] = $rows[0]["lastname"];
			$_SESSION["LAST_ACTIVITY"] = time();
			//echo $rows[0]["umbcid"];
			// update lastaccess for the user and reset the attempt counter
			$db1 -> query("UPDATE users SET lastaccess = SYSDATE() WHERE userid = " . $rows[0]["userid"]);
